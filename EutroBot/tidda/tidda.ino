...
// UNO R4 WiFi uses 12-bit ADC -> 0..4095
...
// HTML dashboard (kept as single raw literal)
...
<div class="wrap">
  ...
  <div class="row" style="margin-top:14px;">
    <div class="card"><div class="label">GPS</div><div class="value" style="font-size:1.2rem"><span id="lat">--</span>, <span id="lng">--</span></div><div id="map"></div>
      <div class="btnbar"><button class="btn" onclick="downloadCSV()">⬇ Download CSV</button></div>
    </div>
    <div class="card">
...
<script>
  ...
  async function refresh(){
    try{
      ...
      // Fixed: use a JS template literal so rotate(...) receives the heading value correctly
...
void sendCSV(WiFiClient& client) {
  ...
  for (int i = 0; i < logCount; i++) {
    ...
    else { client.print(','); client.print(','); } // ensure both latitude and longitude CSV columns exist
    client.println();
  }
}
...
void setup() {
  ...
  digitalWrite(RELAY_PIN, LOW); // ensure known state
  ...
  if (!compass.init()) {
    ...
    // It's recommended to calibrate the compass using a calibration sketch for accurate heading.
    compass.setCalibration(0,0,0,0,0,0); // placeholder; replace with real calibration values after calibration
  }
  ...
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println(F("WiFi connect failed - starting AP fallback"));
    WiFi.softAP("WaterMonitor_AP");
  }
...
void loop() {
  ...
  if (now - lastSampleMs >= SAMPLE_PERIOD_MS) {
    ...
    // relay logic: some relay modules are active LOW. If yours triggers on LOW, invert this logic.
    bool ph_out_of_range = (ph > 8.5 || ph < 6.5);
    bool tds_too_high = (tds > 500);
    bool ntu_too_high = (ntu > 10);

    if (ph_out_of_range || tds_too_high || ntu_too_high) {
